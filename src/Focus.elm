...
create getFunction updateFunction =
    Focus { get = getFunction, update = updateFunction }
...
-}
join : Focus big medium -> Focus medium small -> Focus big small
join (Focus largerFocus) (Focus smallerFocus) =
    let
        joinedGet big =
    ...
    in
    Focus { get = joinedGet, update = joinedUpdate }
